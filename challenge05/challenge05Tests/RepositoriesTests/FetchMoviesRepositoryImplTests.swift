//
//  FetchMoviesRepositoryImplTests.swift
//  challenge05Tests
//
...
class FetchMoviesRepositoryImplTests: XCTestCase {
    private var sut: FetchMoviesRepositoryImpl?
    ...
    private func prepareSut() {
        let mockedEndPoint = MockedFetchMoviesEndPoint()
        sut = FetchMoviesRepositoryImpl(fetchMoviesEndPoint: mockedEndPoint)
    }
    
    //MARK: - test methods
    func testFetchMoviesWithCategory() {
        sut?.fetchMoviesWith(category: .nowPlaying, pageNumber: 1, completion: { [weak self] result in
